...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  return NextResponse.json({
    app_ready: Boolean(process.env.APP_URL),
    webhook_verification_ready: Boolean(process.env.META_WEBHOOK_VERIFY_TOKEN),
    meta_server_credentials_ready: Boolean(process.env.META_APP_SECRET),
    ...
    database_server_ready: Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY),
    environment,
