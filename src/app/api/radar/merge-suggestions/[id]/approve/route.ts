...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await reviewMergeSuggestion(id, "approved", { id: session.id, email: session.email });
    await writeAuditLog({ actorId: session.id, actorEmail: session.email, action: "radar_hub.merge_approved", entityType: "radar_merge_suggestions", entityId: id, summary: "Equivalência aprovada por revisão humana.", metadata: result });
    return NextResponse.json(result);
  } catch (error) { return hubApiError(error, "Falha ao aprovar sugestão."); }
}
